...
ADrawMainWindow::ADrawMainWindow(QWidget *parent) :
    QMainWindow(parent), ui(new Ui::ADrawMainWindow)
{
...
void ADrawMainWindow::createMenuAndToolBar()
{
    ...
    t_FileMenu->addAction(QIcon(QStringLiteral(":/menuImage/image/alarm.png")), tr("关闭(&C)"),[this](){qDebug()<<"关闭";}, QKeySequence::Close);
    t_FileMenu->addSeparator();
    t_FileMenu->addAction(QIcon(QStringLiteral(":/menuImage/image/alarm.png")), tr("保存(&S)"),[this](){qDebug()<<"保存";}, QKeySequence::Save);
    t_FileMenu->addAction(QIcon(QStringLiteral(":/menuImage/image/alarm.png")), tr("删除(&D)"),[this](){qDebug()<<"删除";}, QKeySequence::Delete);
...
//        {
//            InstrumentPanel *customPlot = new InstrumentPanel(QPoint(itemRect.x() + i*300, itemRect.y() + i*300), QRectF(QPointF(0, 0), itemRect.size()));
...
{
    QSqlDatabase db =  AGeneralFunction::getDb(connectName);
    if(db.isValid()) {
        QSqlQuery query(db);
        ...
        foreach(auto item, itemList)
        {
            ...
            QDataStream dataStre(graphicsByte);

            item->serialize(dataStre);
