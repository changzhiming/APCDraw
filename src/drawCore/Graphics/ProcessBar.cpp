...
TProcessBar::TProcessBar(QPointF atScenePos, QRectF bounDingRect, QPen pen, QBrush brush) :
                  TGraphics(atScenePos, bounDingRect, pen, brush)
{
    mType = TItemFactory::ProcessBar;
    if(m_boundingRect.height() >= m_boundingRect.width())
...
void TProcessBar::fDraw(QPainter *painter)
{
    ...
    painter->translate(m_boundingRect.x() + Width / 2, m_boundingRect.y() + Height / 2);

    const int FronSize = 0.12 * qMin(Width, Height) + 1;// 加1，避免为0
