...
{
    //授权码
    AGeneralFunction::addDbCon("QSQLITE", "", m_dbName, "BJM", "chang123." , "Sqlite1");
    QSqlDatabase sql = AGeneralFunction::getDb("Sqlite1");
    QSqlQuery query(sql);
    ...
    if(query.next()) {
        if(query.value("UseTime").toLongLong() >= 3153600) {
            ...
            if (ok && !text.isEmpty()) {
                ...
                if(text == AGeneralFunction::getMd5Hash(AGeneralFunction::getLocalMachineMac().append(QString::number(year)))) {
    ...
    addTimeToDbQuery = QSqlQuery(AGeneralFunction::getDb("Sqlite1"));
    //UI
...
{
    static QSqlQuery query(AGeneralFunction::getDb("Sqlite1"));
        ...
        if(!m_dbName.isEmpty()) {
            ...
            while(query.next()) {
                if(mLineEditName->text() == query.value(0).toString() && mLineEditPassWd->text() == query.value(1).toString()) {
                    ...
                    if(fileInfo.lastRead().toMSecsSinceEpoch() != query.value("CreateTime").toLongLong()) {
